...
async function createProduct(product){
    if (await SupplierRepository.getSupplier(product.supplierId)){//quando nao existe eu volto vazio
...
async function deleteProduct(id){
    ...
    if (sales.length > 0){
        throw new Error("Não é possível excluir o produto porque ele tem vendas cadastradas")
    }
